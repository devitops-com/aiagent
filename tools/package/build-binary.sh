...
# compile would silently be missing from the bundle. Should a dependency ever ship a file
# that cannot compile and is never imported, exclude that path (-x), not every error.
echo "==> Precompiling all modules"
"$PY" -m compileall -q -f -j 0 -s "$STAGE/python" --invalidation-mode unchecked-hash "$STAGE/python"
