...
upstream content from a hash-pinned wheel and cannot disclose this build host. The CycloneDX
SBOMs of jiter, pydantic_core, rpds_py and tokenizers, and tokenizers' compiled extension, for
example, name their own project's CI checkout (/home/runner/work/...), and $HOME/ is
/home/runner/ on a GitHub runner. Such a file is listed in the RECORD of a *.dist-info whose
distribution REQUIREMENTS pins, with a sha256 that matches it. It is still reported on stdout
as upstream content, never as a leak. No other RECORD vouches for anything: not the aiagent
wheel's, which is built from the checkout on this host, and not one that is not the UTF-8 CSV
pip writes.
