...
| Contract with devai | Agreed with the devai session. The reference is devai's [`docs/laya-trainer.md`](https://github.com/ksparavec/devai/blob/main/docs/laya-trainer.md) (merged in ksparavec/devai PR #21). devai-side choices: nulls allowed inside `questions` only; `pool.jsonl` optional on their side; `artifact_id` = 64 hex; `temperature_by_options: {}` with applied temperatures; job ids `ftjob-` + 24 hex. |
