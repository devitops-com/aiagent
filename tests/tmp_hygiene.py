...
/tmp is often a small RAM-backed tmpfs, and the installer, packaging and release tests unpack fake
payloads and create git repos. So each run gets a private directory
``/var/tmp/aiagent-pytest-<pid>-<random>`` holding pytest's basetemp (``tmp_path``, which the
autouse ``clean_env`` fixture takes for every test) and the ``TMPDIR`` of this process and its
children: the scripts under test, git, the installer. The directory is removed when the session
ends, pass or fail. Directories of killed runs (dead pid) are removed when the next run starts;
those of live, concurrent runs are left alone. An explicit ``--basetemp`` becomes the run directory
instead (pytest treats it as disposable too); as with pytest, it must not be a symlink and its
parent must exist.

No project imports, so a test can load it on its own: ``pytest -p tmp_hygiene`` with ``tests/``
on ``sys.path``.
"""
